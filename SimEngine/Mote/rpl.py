...
class Rpl(object):
    ...
    def indicate_preferred_parent_change(self, old_preferred, new_preferred):
        ...
        if new_preferred is None:
            if old_preferred is not None:
                 old_parent_mac_addr = netaddr.EUI(old_preferred)
                 old_parent_ipv6_addr = str(old_parent_mac_addr.ipv6(prefix))
                 # print("\nMote: ", self.mote.id," \n newParent:",'NONE', "\n oldParent:",old_parent_ipv6_addr)
        else:
            new_parent_mac_addr = netaddr.EUI(new_preferred)
            new_parent_ipv6_addr = str(new_parent_mac_addr.ipv6(prefix))
            if old_preferred is not None:
                old_parent_mac_addr = netaddr.EUI(old_preferred)
                old_parent_ipv6_addr = str(old_parent_mac_addr.ipv6(prefix))
                # print("\nMote: ", self.mote.id," \n newParent:",new_parent_ipv6_addr, "\n oldParent:",old_parent_ipv6_addr)
            # else:
                # print("\nMote: ", self.mote.id," \n newParent:",new_parent_ipv6_addr, "\n oldParent:",'NONE')
    ...
    def action_receiveDIO(self, packet):
        ...
        # See Section 14.1. Objective Function Behavior [RFC 6550]
        # Candidate neighbors that are not in the same DODAG are ignored.
        if self.dodagId is not None and self.dodagId != packet[u'app'][u'dodagId']:
            # print('\nMote',self.mote.id,'\nIgnored DIO from other DODAG',packet[u'app'][u'dodagId'])
            return
        ...
        if self.getPreferredParent() is not None:
            ...
            # print('join from action_receiveDIO')
            self.join_dodag(packet[u'app'][u'dodagId'])

    def join_dodag_changed(self, dodagId=None):
        if dodagId is None:
            # DIO promoting NONE dodag
            # re-join the DODAG without receiving a DIO
            # Re-joins self doddag by reseting timers
            assert self.dodagId is not None
            #if our dodag exists rejoin it 
            self.mote.add_ipv6_prefix(d.IPV6_DEFAULT_PREFIX)
            self.trickle_timer.start()
            self.trickle_timer.reset()
            self.stop_dis_timer()
        else:
            #if a dodag is promoted join only if ours is = none
            # do not switch dodag after join one
            # Based on RFC 4.1
            
            if(self.dodagId is  None):
                self.log(
                SimEngine.SimLog.LOG_RPL_JOIN_DAG,
                    {
                        u'_mote_id':  self.mote.id,
                        u'_joined_dag': dodagId,
                    }
                )
                self.mote.add_ipv6_prefix(d.IPV6_DEFAULT_PREFIX)
                self.trickle_timer.start()
                ...
                print('\nMote:',  self.mote.id,'=> Joined DODAG:', dodagId)
    ...
    def join_dodag(self, dodagId=None):
        if dodagId is None:
            ...
            elif self.dodagId != dodagId:
                # print('\nMote:',  self.mote.id,'=> REJOINED DODAG:', dodagId)
            # else:
                print('\nMote:',  self.mote.id,'=> SWITCHED DODAG:', dodagId)
            self.dodagId = dodagId
        self.mote.add_ipv6_prefix(d.IPV6_DEFAULT_PREFIX)
        self.trickle_timer.start()
        self.trickle_timer.reset()
